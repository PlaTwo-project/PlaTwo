...
static const int PIECE_RADIUS = 9;
static const int CLICK_THRESHOLD = 14;

FanoronaPage::FanoronaPage(QWidget* parent)
    : BasePage(parent), chain_active(false), chain_position(-1), current_player_id(0), selected_position(-1), hovered_position(-1)
{
    setMouseTracking(true);

    end_turn_button = new QPushButton("End Turn", this);
    end_turn_button->hide();
    connect(end_turn_button, &QPushButton::clicked, this, [this]() {
        emit moveRequested(-1, -1, -1);
    });
}
...
void FanoronaPage::positionEndTurnButton() {

    QSize hint = end_turn_button->sizeHint();
    int btnW = hint.width();
    QPoint corner(margin_offset + (FanoronaBoard::COLS - 1) * cell_spacing - btnW + 20, margin_offset - 40);
    end_turn_button->move(corner);
}
...
void FanoronaPage::updateFromGame(const Game* game) {
    ...
    end_turn_button->setVisible(chain_active && is_input_enabled);
...
void FanoronaPage::paintEvent(QPaintEvent* event) {
    ...
    for (int position = 0; position < FanoronaBoard::TOTAL_POSITIONS; ++position) {
        ...
        if (occupant == 0) {
            ...
            painter.setPen(Qt::black);
            painter.setBrush(occupant == 1 ? QColor(100, 149, 237) : QColor(255, 99, 71));
