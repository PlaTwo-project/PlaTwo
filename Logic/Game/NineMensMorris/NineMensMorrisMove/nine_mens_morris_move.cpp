...
NineMensMorrisMove::NineMensMorrisMove(MoveType move_type, PlayerSlot player_id, int from_position, int to_position)
...
QByteArray NineMensMorrisMove::serializeMove() const {
    return QString("%1,%2,%3,%4").arg(static_cast<int>(move_type)).arg(static_cast<int>(player_id)).arg(from_position).arg(to_position).toUtf8();
}
...
PlayerSlot NineMensMorrisMove::getPlayerId() const {
    return player_id;
}
